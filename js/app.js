...
const customEl = document.querySelector(".calculator__group-tips-amount-custom")
const customInputEl = document.querySelector(".calculator__group-tips-amount-custom-input")
...
class Calculator {
  ...
  updateUI() {
    ...
    this.total > 0 ? resetBtn.classList.remove("disabled") : resetBtn.classList.add("disabled")
  }
...
billEl.addEventListener("input", e => {
  // Add bill 
  tipCalculator.bill = isNaN(parseFloat(e.target.value)) ? 0 : parseFloat(e.target.value)
...
tipOptions.addEventListener("click", e => {
  if (!e.target.classList.contains("selected") && !e.target.className.includes("custom")) {
